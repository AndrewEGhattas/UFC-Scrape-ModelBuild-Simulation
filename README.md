...
## High-level notes
1. Note that, if you are just interested in the resulting data, and don't want to run any programs. The final dataset "5a_AUF.csv" has been uploaded to GitHub. Keep in mind that this file is current up until 2/24/2022; fights which occur after this time will not be in "5a_AUF.csv", you'll need to run the programs to generate the most up to date dataset. Alternatively, if you do run the programs, then you can read in  5a_DataPlusHelperVars.R, and use the AUF dataframe.
...
4. I have code which implements the betting strategy, printing out radar plots, and expected gain graphs. However, I can't find a consistently profitable betting strategy, so this output is not currently included in the committed set of programs
...
1. <ROOT>/H2O Models/
	- This directory contains the H2O output models, that will be called upon to build the betting strategies within the program
2. <ROOT>/HypParam Plots/
	- For each model type fit (GBM, GLM, NBC, NN, RF), there are multiple hyperparameters
	-  The plots in this directory summarize the distribution of validation set logloss, for each level of the tested hyperparameters
	-  The triangle points, and line in the figures display the mean validation logloss value
3. <ROOT>/Logs/
	-  The log files for each program called
4. <ROOT>/Model metadata/
	-  This file has an entry for each model, capturing its model type (GBM, GLM, NBC, NN, RF), the hyperparameters used, and the training/validation logloss
5. <ROOT>/Parameters and functions/
	-  Program parameters and utility functions created in the main control program, and called on for subsequent sub-program calls
6. <ROOT>/Processed data/
	-  This directory contains the interim output dataframes for each program
	-  The data is stored as .R and .csv. The .R files are actually used throughout the program calls, whereas the .csv files are useful for opening and visually inspecting in order to bug-check
	-  Note that, if you want to build your own models, using the final dataframe, but not the modeling framework proposed in these programs, then you can read in 5a_DataPlusHelperVars.R, and use the AUF dataframe, or equivalently, read in 5a_AUF.csv
7. <ROOT>/Strategy graphics/
	- These graphics are the results of back-testing different betting strategies
	- First off, different models are used in each of these strategies,
		- Top<X> denotes the top X models, where "top" is determined by the smallest validation logloss
		- The 2nd portion of the graphic titles correspond to which modeling framework is being considered: GBM, GLM, NBC, NN, RF, and "Any" denotes any of these model types
		- Ex. Top100_GBM represents the back-tested betting strategy results using an ensemble of the 100 best GBM models
		- Ex. Top25_Any represents the back-tested betting strategy results using an ensemble of the 25 best models across all model types (GBM, GLM, NBC, NN, RF)
	- General description of the betting simulation
		- Based on model/ensemble prediction, and actual betting odds, an expected return is calculated based on betting on either fighter
		- A bet can be accepted if the expected return is over some threshold "Y".
		- We will look at various choices for "Y"
	- The 1st panel of the graphic,
		- Shows the simulated gain over time, across all the fights that we have the opportunity to bet on
		- there isn't a point for each fight, because the betting strategy may not want to bet on the fight
		- There are multiple lines, to illustrate betting strategies having different values of "Y"
		- Only a subset of all values of "Y" are displayed, those having the highest final return
	- The 2nd panel of the graphic,
		- Shows the final simulated return as a function of decision threshold "Y"
		- The actual line is shown in red, and a few smoothing lines are overlayed on top of it

## The programs in this process are described below
1. UFC_ControlProgram.R
	- Program description: Main control program that creates program parameters and calls the other programs.
	- Directories created
		- Creates directory which will contain program parameters and utility functions
			- <ROOT>/Parameters and functions/
		- Creates directory which will capture the logs for each program
			- <ROOT>/Logs/
	- Outputs
		- Creates an R file containing all program parameters and utility functions: <ROOT>/Parameters and functions/ParamsAndFuncs.R
2. UFC_Program1.R
	- Program description: Scrape the fight level data from UFC.com
	- Directories created
		- Creates directory which will contain all output data: <ROOT>/Processed data/
	- Outputs
		- Log
			- <ROOT>/Logs/UFC_Program1.txt
		- List of scraped fighter URLs
			- <ROOT>/Processed data/1a_UFC_Fighter_pages.csv
			- <ROOT>/Processed data/1a_UFC_Fighter_pages.R
		- List of scraped fight stats
			- <ROOT>/Processed data/1b_All_UFC_Fights.csv
			- <ROOT>/Processed data/1b_All_UFC_Fights.R
3. UFC_Program2.R
	- Program description: Scrape the historical betting odds
	- Directories created (temporary)
		- Creates directory which will contain all temporary output, deleted at end of program
			- <ROOT>/Processed data/temp/
	- Outputs
		- Log
			- <ROOT>/Logs/UFC_Program2.txt
		- List of scraped bettings odds URLS
			- <ROOT>/Processed data/2a_All_Odds_Events_URLs.csv
			- <ROOT>/Processed data/2a_All_Odds_Events_URLs.R
		- List of fight level odds, scraped from each URL
			- <ROOT>/Processed data/2b_All_Odds.csv
			- <ROOT>/Processed data/2b_All_Odds.R
		- Processed fight level odds data
			- <ROOT>/Processed data/2c_Final_Odds.csv
			- <ROOT>/Processed data/2c_Final_Odds.R
		- Merged odds data to fight level stats
			- <ROOT>/Processed data/2d_All_UFC_Fights.csv
			- <ROOT>/Processed data/2d_All_UFC_Fights.R
4. UFC_Program3.R
	- Program description: Scrape the historical fighter rankings
	- Directories created (temporary)
		- Creates directory which will contain all temporary output, deleted at end of program
			- <ROOT>/Processed data/temp/
	- Outputs
		...
		- List of scraped historical rankings over time
			- <ROOT>/Processed data/3a_All_Rankings.csv
			- <ROOT>/Processed data/3a_All_Rankings.R
		- Processed historical rankings dataframe
		...
		- Missing values imputed (when possible)
			- <ROOT>/Processed data/3d_All_UFC_Fights.csv
			- <ROOT>/Processed data/3d_All_UFC_Fights.R
5. UFC_Program4.R
	- Program description: Combine the fights, rankings, and odds data then feature engineer/process the dataframe
	- Directories created (temporary)
		- Creates directory which will contain all temporary output, deleted at end of program
			- <ROOT>/Processed data/temp/
	- Outputs
...
6. UFC_Program5.R
	- Program description: Fit and use the ML models for simulating the betting strategy. This program can be run, the hyperparameter tuning plots observed, hyperparameter values adjusted, then program rerun. This can be continued until the user feels they've properly explored the hyperparameter space. If you do this, then the program will simply append the new models to existing directories, you won't lose the models that were already fit/saved.
	- Directories created (temporary)
		- Creates directory which will contain all temporary output, deleted at end of program
			- <ROOT>/Processed data/temp/
	- Directories created
		- Creates directory which will contain all H2O model objects: <ROOT>/H2O Models/
		- Creates directory which will contain all hyperparameter tuning plots: <ROOT>/HypParam Plots/
		- Creates directory which will contain aggregated model metadata and performance: <ROOT>/Model metadata/
		- Creates directory which will contain betting simulation results: <ROOT>/Strategy graphics/
	- Outputs
		- Log
		...
		- Final dataframe, vector of predictors, and object naming the target variable. Note that, if you want to build your own models, using the final dataframe, but not the modeling framework proposed in these programs, then you can read in 5a_DataPlusHelperVars.R, and use the AUF dataframe, or equivalently, read in 5a_AUF.csv
			- <ROOT>/Processed data/5a_DataPlusHelperVars.R
			- <ROOT>/Processed data/5a_AUF.csv
		- Model metadata read stored as dataframes
			- <ROOT>/Processed data/5c_ModelMetadata.R
		- Betting strategy graphics
			- <ROOT>/Strategy graphics/<BETTINGSTRAT>

## Data Dictionary for final data (5a_AUF.csv or AUF dataframe within 5a_DataPlusHelperVars.R created by programs
...
- F_SSL_Beta: Using previous 10 fights, normalize this fighter's significant strikes landed (by time in case of stoppage) for each round, then estimate slope of line to assess how significant strikes landed changes over round, on average
...
- F_Opp_SSL_Beta: Using the fighter's previous 10 opponents, normalize opponent significant strikes landed (by time in case of stoppage) for each round, then estimate slope of line to assess how significant strikes landed changes over round, on average

### Opponent level variables
...
- O_SSL_Beta: Using previous 10 fights, normalize this opponent's significant strikes landed (by time in case of stoppage) for each round, then estimate slope of line to assess how significant strikes landed changes over round, on average
...
- O_Opp_SSL_Beta: Using the opponent's previous 10 opponents, normalize those opponents' significant strikes landed (by time in case of stoppage) for each round, then estimate slope of line to assess how significant strikes landed changes over round, on average
